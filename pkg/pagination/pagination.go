...
func New(ctx *gin.Context, db *gorm.DB, baseURL string, PerPage int) *Pagination {
	...
	// 设置当前页码
	p.SetPage(p.GetPageFromRequest(ctx))
...
func (p Pagination) GetPageFromRequest(ctx *gin.Context) int {
	page := ctx.Param(config.GetString("pagination.url_query"))
