...
// Show 显示分类下的文章列表
func (cc *CategoriesController) Show(ctx *gin.Context) {
	...
	// 3. 获取结果集
	articles, pagerData, err := article.GetByCategoryID(_category.GetStringID(), ctx, 2)

	if err != nil {
		cc.ResposeForSQLError(ctx.Writer, err)
	} else {
